        ...
        function results = process(obj,waveform,chanBW,varargin)
            ...
            while state.nextState ~= "NoPacketDetected"
                switch state.nextState
                    case "Preamble"
                        % fprintf('Waveform processing update: %s\n', state.nextState);
                        fprintf('.');
                    ...
                    case "L-SIG"
                        ...
                        format = wlanFormatDetect(fmtDetectBuffer(commonIndices.LSIG(1):end,:),preRes.ChanEstNonHT,preRes.NoiseEstNonHT,chanBW,"SuppressWarnings",true);
...
function packetRes = initializeResults(format,packetNum,preRes,lsigRes,osf)
    ...
    if any(strcmp(format,["HE-SU","HE-MU","HE-EXT-SU","HE-TB","EHT-MU","EHT-TB"]))
